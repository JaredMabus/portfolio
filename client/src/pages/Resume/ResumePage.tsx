...
import {resumeData, ResumeDataType, TechCategoryType, JobsType, InstitutionsType }from "./resumeData";
...
const TechnicalSkills: React.FC<Props> = ({ data }) => {
  ...
  return (
    ...
    >
      {data.skills && (data.skills.map((tech:TechCategoryType ) => (
        <Stack
          key={tech.category}
          sx={{
            alignItems: "center",
    ...
    >
      {data.jobs && (data.jobs.map((job) => (
        <Stack key={job.employer} sx={{ width: "100%" }}>
          ...
          <Box sx={{ p: 0.5, pl: 3 }}>
            <ul>
              {job.content && (job.content.map((item: string) => (
                <li key={item}>
...
const Education: React.FC<Props> = ({ data }) => {
  ...
  return (
    ...
    >
      {data.institution && (data.institution.map((ist) => (
        ...
        >
          ...
          >
            <Link
              ...
              onClick={() => (window.location.assign("mailto:jwmabusth@gmail.com"))}
            >
              jwmabusth@gmail.com
            </Link>
            <a
